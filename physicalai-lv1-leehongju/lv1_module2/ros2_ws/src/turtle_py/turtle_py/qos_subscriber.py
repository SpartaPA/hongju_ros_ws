...
class QosSubscriber(Node):
    ...
    def __init__(self):
        ...
        # 원점 거리가 이 값을 초과하면 WARN 로그 출력 (Float32 거리 토픽에만 적용)
    ...
    def _on_msg(self, msg):
        ...
        if hasattr(msg, 'waypoints'):
            ...
            # Float32 거리 토픽: warn_distance 초과 시 WARN, 아니면 INFO
            if msg.data > self._warn_distance:
                self.get_logger().warn(
                    f'#{self._count_total} 원점 거리 초과! 현재: {msg.data:.3f}m > '
                    f'임계값: {self._warn_distance:.3f}m'
                )
            else:
