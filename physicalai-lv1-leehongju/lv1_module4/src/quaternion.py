...
def matrix_to_quaternion(R) -> np.ndarray:
    ...
    if R.shape != (3, 3):
        raise ValueError(f"R 은 (3, 3) 행렬이어야 합니다. 입력 shape={R.shape}")
...
def quaternion_to_matrix(q) -> np.ndarray:
    ...
    if q.shape != (4,):
        raise ValueError(f"q 는 길이 4의 배열이어야 합니다. 입력 shape={q.shape}")

    norm = np.linalg.norm(q)
    if norm < 1e-12:
        raise ValueError("쿼터니언의 크기가 0에 가깝습니다.")
    q = q / norm
...
def quat_angle(q0, q1) -> float:
    ...
    q1 = np.asarray(q1, dtype=float).flatten()
...
def slerp(q0, q1, t: float, eps: float = 1e-8) -> np.ndarray:
    ...
    # 두 쿼터니언이 거의 같은 경우 (Zero Division 방지 -> LERP 후 정규화)
    if d > 1.0 - eps:
    ...
    q_interp = (np.sin((1.0 - t) * omega) * q0 + np.sin(t * omega) * q1) / sin_omega
    return q_interp / np.linalg.norm(q_interp)
