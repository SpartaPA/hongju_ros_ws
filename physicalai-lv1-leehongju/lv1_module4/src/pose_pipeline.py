...
class PosePipeline:
    ...
    def __init__(self, T_base_link, T_link_camera, joint_axis: str = "z"):
        ...
        T_base_link_arr = np.asarray(T_base_link, dtype=float)
        T_link_camera_arr = np.asarray(T_link_camera, dtype=float)

        if T_base_link_arr.shape != (4, 4):
            raise ValueError(f"T_base_link는 (4,4) 형상이어야 합니다. 입력: {T_base_link_arr.shape}")
        if T_link_camera_arr.shape != (4, 4):
            raise ValueError(f"T_link_camera는 (4,4) 형상이어야 합니다. 입력: {T_link_camera_arr.shape}")
        if joint_axis not in _ROT:
            raise ValueError(f"joint_axis는 'x', 'y', 'z' 중 하나여야 합니다. 입력: {joint_axis}")

        self._T_base_link0 = T_base_link_arr
        self._T_link_camera = T_link_camera_arr
    ...
    def T_base_link(self) -> np.ndarray:
        ...
        """
        rot_fn = _ROT[self.joint_axis]
        R_joint = rot_fn(self.joint_angle)
        T_joint = make_T(R_joint, [0.0, 0.0, 0.0])
