...
4. **두 장치를 구분한 속성**: 라이다 `KERNEL=="loop22", SYMLINK+="robot_lidar"` / IMU `KERNEL=="loop23", SYMLINK+="robot_imu"`
   ...
   **loop/backing_file**
   메모리 상에 존재하는 커널 상태 정보
```
cat /sys/block/loop23/loop/backing_file
/home/pa8/fake_sensors/lidar.img
losetup -a
/dev/loop23: []: (/home/pa8/fake_sensors/lidar.img)
/dev/loop24: []: (/home/pa8/fake_sensors/imu.img)
```
...
6. **순서를 바꿔 재연결한 뒤 `ls -l /dev/robot_*` 결과**
   ```
   ls -l /dev/robot_*
   lrwxrwxrwx 1 root root 6 Sep  4 10:30 /dev/robot_imu -> loop23
   lrwxrwxrwx 1 root root 6 Sep  4 10:30 /dev/robot_lidar -> loop22
   ```
   ...
   sudo udevadm trigger
   ls -l /dev/robot_*
   lrwxrwxrwx 1 root root 6 Sep  4 10:38 /dev/robot_imu -> loop23
   lrwxrwxrwx 1 root root 6 Sep  4 10:38 /dev/robot_lidar -> loop22
   ```
...
1. **저장소 URL**: `https://github.com/kanichong/lv1_module1_temp.git` / **PR URL**: `https://github.com/kanichong/lv1_module1_temp/pull/1#issue-5309153406`
2. **PR 리뷰 코멘트와 반영 커밋** (캡처 또는 링크)
   ![[Pasted image 20260901164206.png]]
3. **충돌이 난 파일과 줄**: `___` — 충돌 표식의 뜻과 해결 방법
   ...
   >>>>>>> branch-b
   ```
   충돌 내용을 수정하여 올바른 코드를 적용하여 다시 커밋.
   
1. **merge 방식 이력 그래프** / **rebase 방식 이력 그래프** (두 출력 비교)
   ```
* 8cebb0e (HEAD -> branch-b) feat: B브랜치 README update
*   d78a808 (origin/main, main) Merge pull request #1 from kanichong/feature/udev-rules
|\  
| * 16e2ec0 (origin/feature/udev-rules, feature/udev-rules) feat: Add udev rules and key reference table
|/  
* b6aed46 (feature/compute-layout) Init: 배달 로봇 사양(모듈1_문제1)
   ```
```
*   a49b7a4 (HEAD -> branch-a, origin/branch-a) fix: A브랜치와 B브랜치 conflict 해결.
|\  
| * 8cebb0e (branch-b) feat: B브랜치 README update
* | 8cb35ff feat: A브랜치 README update
|/  
*   d78a808 (origin/main, main) Merge pull request #1 from kanichong/feature/udev-rules
|\  
| * 16e2ec0 (origin/feature/udev-rules, feature/udev-rules) feat: Add udev rules and key reference table
...
```   
   ...
   merge 방식은 다이아몬드 형태의 분기 및 병합 구조가 만들어진다. 실제로 작업이 분기되고 병합된 역사적 사실이 명확히 보존됨. 브랜치가 많아지면 그래프가 매우 복잡해짐.
   rebase-merge 방식은 단일 직렬(Linear) 커밋 이력이 만들어진다. 히스토리가 일직선이라 추적·검색·`git bisect` 버그 탐색이 매우 쉬움. 이미 원격에 공유된 브랜치에 쓰면 충돌 재발생 및 이력 꼬임 위험.
   
   
2. **언제 merge 를, 언제 rebase 를 쓸지** — 3줄 이내
   
   Merge와 Rebase의 선호도는 "히스토리의 보존(Merge)"과 "히스토리의 가독성(Rebase)" 중 무엇을 가치 있게 두느냐에 따라 결정.
   한 번이라도 Push 되어 다른 사람이 쪼개어 작업 중인 브랜치에는 절대로 Rebase를 쓰지 않는다.
   기능 단위 상위 브랜치 병합 시에는 `git merge --no-ff`를 사용하여 히스토리를 보존한다.
